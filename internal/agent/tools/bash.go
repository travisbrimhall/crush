...
func effectiveBannedCommands(unbanCommands []string) []string {
	if len(unbanCommands) == 0 {
		return bannedCommands
	}
	unbanSet := make(map[string]struct{}, len(unbanCommands))
	for _, cmd := range unbanCommands {
		unbanSet[cmd] = struct{}{}
	}
	var filtered []string
	for _, cmd := range bannedCommands {
		if _, unbanned := unbanSet[cmd]; !unbanned {
...
func bashDescription(attribution *config.Attribution, modelName string, unbanCommands []string) string {
	bannedCommandsStr := strings.Join(effectiveBannedCommands(unbanCommands), ", ")
...
func blockFuncs(unbanCommands []string) []shell.BlockFunc {
	return []shell.BlockFunc{
		shell.CommandsBlocker(effectiveBannedCommands(unbanCommands)),
...
func NewBashTool(permissions permission.Service, workingDir string, attribution *config.Attribution, modelName string, unbanCommands []string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		BashToolName,
		string(bashDescription(attribution, modelName, unbanCommands)),
		func(ctx context.Context, params BashParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if params.RunInBackground {
				...
				bgShell, err := bgManager.Start(context.Background(), execWorkingDir, blockFuncs(unbanCommands), params.Command, params.Description)
				if err != nil {
